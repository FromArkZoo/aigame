...
**Helper:** `experiments/field_connect_probe/eval_helper.py --game Q` (run `--rules` first for rules; `--control` for influence map).
...
All moves engine-verified through `eval_helper.py --game Q`. Action IDs = cell indices (q + 22*r) for placement; pass=484; pie_swap=485.
...
*Output saved to `/Users/jamesbrowne/aigame/evaluations/field_connect_probe/team-{{TEAM_N}}_gameQ.md`.*
