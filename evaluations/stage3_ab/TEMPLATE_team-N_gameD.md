...
**Helper:** `evaluations/stage3_ab/play.py --game D` (run `--rules` first; `--control` for influence map).
...
All moves engine-verified through `play.py --game D`. Action IDs = cell indices (q + 22*r); pass=484; swap=485 (if pie rule is on).
...
*Output saved to `evaluations/stage3_ab/team-{{TEAM_N}}_gameD.md`.*
