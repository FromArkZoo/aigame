...
**Helper:** `evaluations/siege_ab/play.py --game X` (run `--rules` first; `--control` for influence map).
...
(b) {{Capture analog: surround → Go? Outnumber → Tafl/Ataxx? Custodian → Othello/Reversi?}}
...
*Output saved to `evaluations/siege_ab/team-{{TEAM_N}}_gameX.md`.*
