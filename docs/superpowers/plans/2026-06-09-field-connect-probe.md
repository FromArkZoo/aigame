...
- **Known risks accepted:** decisive-at-final-step counted as timeout (noted in code); `trainer.evaluate()` diagnostic key name `trained_vs_random_winrate` taken from run.py's return schema — if `trainer.evaluate` uses a different key, fall back to `diag.get("trained_vs_random", ...)` (guarded with `.get` + sentinel −1.0 already).
