...
## Agent-team evaluation results (campaign 2026-06-06)

Full synthesis: `evaluations/run21/SUMMARY.md` (35 verdicts, 5 independent teams). Pre-eval komi gate: `experiments/r21_komi_calibration/r21_komi_calibrated.md` (6/7 balanced <0.10; `573562833174` rush-broken). Post-R21 deep analysis + decision rule: `analysis_post_r21.md`.

**Headline: no game cleared Overall 5.0 — G1 FAILS, unanimously.** Campaign mean **3.69** (team means 3.53–3.99), tied with R20 production (3.73), below R19 (4.375) and the R8 replay anchor (4.10).

| Game (GE) | mean Overall | SD |
|---|---:|---:|
| menger `e1453` (GE-top 0.177) | 3.66 | 0.27 |
| menger `e52e` (0.138) | 3.68 | 0.31 |
| menger `bfd1` (0.126) | 3.68 | 0.30 |
| menger `1fea` (0.118) | 3.50 | 0.29 |
| carpet `d995` (0.103) | 3.78 | 0.32 |
| grid `b12` (0.099) | 3.72 | 0.31 |
| grid `573` (connection, 0.002) | 3.78 | 0.36 |

**GE rank is inverted at the extremes**: GE-#1 `e1453` ranks 6th by agents; GE-last `573` ties for 1st. The §"Methodological wins" bimodal-noise framing earlier in this report is **superseded** — the GE noise is small-sample evaluation-estimator noise, not PPO training failure (Probe B, `experiments/r21_variance_probe/`), and the planned R22 PPO-convergence filter is scrapped (survivorship-biased).

**Goals**: G1 ❌; G2 nominal-PASS/functional-FAIL (the 4 menger games play as 1–2 games — komi + inert max_turns are the only diffs); G3 mixed (6/7 balanced, `573` rush-broken); G6 ❌ (3.78) though the connection *design* is the slate's most interactive (custodian capture fires + cuts paths — an upgrade over R8's never-firing surround).

**G7 / decision rule FIRES → PIVOT + SATURATION.** No game >5.0 and the cross-run mean did not rise; R20 (4.80) + R21 (3.78) are two consecutive sub-5.0 campaigns. The rule-grammar + GE-evolution regime is empirically saturated. Constructive next direction (NOT another GE run, NOT MCTS-over-the-same-GE): bigger/connection-relevant boards (hex degree-6) + connection-win with permanent-cut (surround) capture + influence-in-win-logic + Quality-Diversity selection. Details in `evaluations/run21/SUMMARY.md` § Decision.
